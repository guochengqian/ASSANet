...
from .data_utils import grid_subsampling, PointcloudToTensor, PointcloudRandomRotate, PointcloudScaleAndJitter
...
def build_s3dis_loader(config):
    # set the data loader
    train_transforms = transforms.Compose([
        PointcloudToTensor(),
        PointcloudRandomRotate(x_range=config.data.x_angle_range, y_range=config.data.y_angle_range,
                               z_range=config.data.z_angle_range),
        PointcloudScaleAndJitter(scale_low=config.data.scale_low, scale_high=config.data.scale_high,
    ...
    train_dataset = S3DISSeg(input_features_dim=config.data.input_features_dim,
                             subsampling_parameter=config.data.sampleDl, color_drop=config.data.color_drop,
                             in_radius=config.data.in_radius, num_points=config.data.num_points,
                             num_steps=config.data.num_steps, num_epochs=config.epochs,
                             data_root=config.data.data_root, transforms=train_transforms,
                             split='train')
    val_dataset = S3DISSeg(input_features_dim=config.data.input_features_dim,
                           subsampling_parameter=config.data.sampleDl, color_drop=config.data.color_drop,
                           in_radius=config.data.in_radius, num_points=config.data.num_points,
                           num_steps=config.data.num_steps, num_epochs=20,
                           data_root=config.data.data_root, transforms=test_transforms,
                           split='val')
